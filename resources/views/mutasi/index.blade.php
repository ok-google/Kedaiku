...
@section('content')
                    ...
                    <form action="#">
                        ...
                        <div class="form-group-inner" align="left">
                            ...
                            <div class="form-select-list">
                                <select class="form-control custom-select-value" name="debet" id="debet">
                        ...
                        <div class="form-group-inner" align="left">
                            ...
                            <div class="form-select-list">
                                <select class="form-control custom-select-value" name="kredit" id="kredit">
                                    @foreach ($Kas as $value)
                                        <option value="{{ $value->id }}">{{ $value->nama }}</option>    
                                    @endforeach
                                </select>
    ...
    <script>
        function insert(e){
            ...
            $.ajax({
                url: "{{ url('/transaksi/mutasi/store') }}",
                method: 'post',
                data: {
                    _token: $("input[name=_token]").val(),
                    KasDebetId: $('#debet').val(),
                    KasKreditId: $('#kredit').val(),
                    Tanggal: $('#tgl').val(),
                    Jumlah: $('#jumlah').val()
                },
        ...
        function getData(){
            $.ajax({
                url: "{{ url('/transaksi/mutasi/getAll') }}",
                method: 'get',
                data: {
                    start: $('#start').val(),
                    end: $('#end').val(),
                        ...
                        var html = '<div class="breadcome-list shadow-reset">' +
                                        '<div class="row">' +
                                            ...
                                            '<div style="margin-left: 2%; font-size: 12px; font-weight: black; font-style: bold;">' +
                                                'Kas Debet : ' + result[index].NamaKasDebet +                                 
                                            '</div>' +
                                            '<div style="margin-left: 2%; font-size: 12px; font-weight: black; font-style: bold;">' +
                                                'Kas Kredit : ' + result[index].NamaKasKredit +
                                            '</div>' +
                                            '<div style="margin-left: 2%; font-size: 12px; font-weight: black; font-style: bold;">' +
                                                'Rp ' + numberSeparator(result[index].jumlah_debet) +
                                            '</div>' +
                                        '</div>' +
                                    '</div>';

                        $('#listMutasi').append(html);
                    });
                }
            });   
        }
